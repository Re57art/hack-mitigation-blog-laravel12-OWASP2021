        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                <div class="card shadow-lg mb-5 border-0 p-4 text-center">
                    <div class="d-flex flex-column align-items-center">
                        <div class="profile-avatar mb-3">
                            <img
                                src="{{$user->avatar ? Storage::url("public/images/users/$user->id/" . $user->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=2ECC71&color=fff'}}"
                            ...
                            @foreach ($user->articles as $article)
                                <tr>
                                    ...
                                    <td><a href="{{route('articles.show',$article->id)}}"
                                           class="fw-semibold text-primary">{{$article->title}}</a></td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{route('articles.edit',$article->id)}}"
                                           class="btn btn-sm btn-outline-warning me-2" title="Edit"><i
                    ...
                    <form method="POST" action="{{route('users.update',$user->id)}}">
                        ...
                        <div class="mb-4">
                            ...
                            <input class="form-control rounded-pill" type="text" name="name"
                                   value="{{old('name') ?? $user->name}}" placeholder="You really change your name?!">
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-semibold">Email</label>
                            <input class="form-control rounded-pill" type="email" name="email"
                                   placeholder="Your brand new email" value="{{old('email') ?? $user->email}}">
                        </div>
                ...
                <div class="card shadow-sm border-0 p-4 mb-4">
                    ...
                    <a href="{{route('download','filename=privacy.pdf')}}"
                       class="d-block mb-2 text-decoration-none text-info fw-semibold"><i
                            class="bi bi-file-earmark-pdf me-2"></i>Privacy policy</a>
                    <a href="{{route('download','filename=cookie-policy.pdf')}}"
                       class="d-block text-decoration-none text-info fw-semibold"><i
                            class="bi bi-file-earmark-pdf me-2"></i>Cookie policy</a>
                </div>
